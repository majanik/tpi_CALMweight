...
int main(int argc, char **argv)
{
  ...
  CorrFctnDirectYlm *fS = new CorrFctnDirectYlm("Sfun",3,NBINS,0.0,RMAX);
  CorrFctnDirectYlm *fK = new CorrFctnDirectYlm("Kfun",3,NBINS,0.0,RMAX);

  CorrFctnDirectYlm *fU = new CorrFctnDirectYlm("Ufun",3,NBINS,0.0,RMAX);
  ...
  for (int iter=0; iter<NSAMPLES*2; iter++) {
    double x = fRand->Rndm()*2*RMAX-RMAX;
    double y = fRand->Rndm()*2*RMAX-RMAX;
    double z = fRand->Rndm()*2*RMAX-RMAX;

    if (sqrt(x*x+y*y+z*z) < RMAX) {
      fS->AddMixedPair(x,y,z,1.0);
      fK->AddMixedPair(x,y,z,1.0);
    }
  }
